...
public class RegisterView {
    ...
    private VBox createRegisterForm() {
        ...
        addFormField(grid, "Nom complet", 0, 0, 2);
        addFormField(grid, "Email", 0, 1, 1);
        addPasswordField(grid, "Mot de passe", 1, 1, 1);
        addFormField(grid, "Numéro de permis", 0, 2, 2);
        addFormField(grid, "Téléphone", 0, 3, 1);
        addFormField(grid, "Adresse", 1, 3, 1);
        ...
        btnRegister.setOnAction(e -> app.Main.showCatalogueView());
        ...
        linkLogin.setOnAction(e -> app.Main.showLoginView());
        GridPane.setColumnSpan(linkLogin, 2);
        grid.add(linkLogin, 0, 5);

        form.getChildren().addAll(formIcon, title, grid);
        return form;
    }

    private void addFormField(GridPane grid, String placeholder, int col, int row, int colspan) {
        TextField field = new TextField();
        field.setPromptText(placeholder);
        field.setStyle(getFieldStyle());
        field.setPrefWidth(250);
        grid.add(field, col, row, colspan, 1);
    }

    private void addPasswordField(GridPane grid, String placeholder, int col, int row, int colspan) {
